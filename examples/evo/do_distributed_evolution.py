...
args.total_prompts_per_epoch = args.total_parallel_generations // args.generations_per_prompt
    
###### Evolution Model Implementation

def fold_in_helper(key):
    return jax.random.fold_in(key[0], key[3] * args.total_parallel_generations + key[2])
...
def simple_full_update(param, key, scores, lr):
    ...
    # true_key, sigma = key
    sigma = key[1]
    noises = jax.vmap(partial(jax.random.normal, shape=param.shape, dtype=param.dtype))(jax.vmap(fold_in_helper)(key))
...
def simple_lora_update(param, key, scores, lr):
    ...
    # true_key, sigma = key
    sigma = key[1]
    noises = jax.vmap(partial(jax.random.normal, shape=(a+b, args.lora_dim), dtype=param.dtype))(jax.vmap(fold_in_helper)(key))
    Bs = noises[:, :b] # Bxbxl
...
def _do_update(params, model_keys, raw_scores, lr):
    ...
    def inner_update(param, model_key, lora_map_ans):
        ...
        if len(model_key[0].shape) == 1:
            ...
            return jax.lax.scan(lambda _, x: (0, update_fn(x[0], x[1], true_scores, lr)), 0, xs=(param, tuple([jnp.moveaxis(q, 0, 1) for q in model_key])))[1]
...
def evo_lora2(M, param, key):
    ...
    a, b = param.shape
    lora_params = jax.random.normal(fold_in_helper(key), (a+b, args.lora_dim), dtype=param.dtype) * key[1]
    B = lora_params[:b]
...
def evo(param, key):
    if args.freeze_nonlora:
        return param
    return param + jax.random.normal(fold_in_helper(key), param.shape, dtype=param.dtype) * key[1] # r_k is exception
...
def generate_base_model_tree_keys(params, base_model_key):
...
def single_generate_model_tree_keys(base_model_tree_keys, thread_id, epoch):
    sigma_antithetic = jnp.where(thread_id % 2 == 0, args.evo_sigma, -args.evo_sigma) if args.use_antithetic else args.evo_sigma
    return jax.tree.map(lambda x: (x,
                                   jnp.ones(x.shape, dtype=sigma_antithetic.dtype) * sigma_antithetic,
                                   jnp.ones(x.shape, dtype=thread_id.dtype) * (thread_id // 2 if args.use_antithetic else thread_id),
                                   jnp.ones(x.shape, dtype=epoch.dtype) * epoch
                                   ), base_model_tree_keys)
...
def single_generate_keys_for_generation(base_gen_key, thread_id, epoch):
    # Bizarre jax bug, can't do double fold-in with multi-node
    # return jax.random.fold_in(jax.random.fold_in(base_gen_key, epoch), thread_id // 2 if args.use_antithetic else thread_id)
    
    return jax.random.fold_in(base_gen_key, epoch * args.total_parallel_generations + (thread_id // 2 if args.use_antithetic else thread_id))
    # if args.use_antithetic:
    #     gen_key = jax.random.fold_in(jax.random.fold_in(original_key, epoch), thread_id // 2)
    #     sigma_antithetic = jnp.where(thread_id % 2 == 0, args.evo_sigma, -args.evo_sigma)
    #     evo_key = (thread_id // 2)
    # else:
    #     gen_key = jax.random.fold_in(jax.random.fold_in(original_key, epoch), thread_id)
    #     sigma_antithetic = args.evo_sigma
    #     evo_key = thread_id
    # return evo_key, gen_key, sigma_antithetic
...
# head_param = params['head']['weight']

# head_param = jax.make_array_from_single_device_arrays(head_param.shape, NamedSharding(mesh, P()), [jax.device_put(head_param, d) for d in jax.local_devices()])

# print(head_param.device)
...
# params = [jax.device_put(params, d) for d in jax.local_devices()]
...
# params = jax.device_put(params, NamedSharding(mesh, P())) # replicated across all GPUs

init_state = shard_map(partial(RWKV.default_state, config=config), mesh=mesh, in_specs=P(), out_specs=P())(params) # replicated across all GPUs
# init_states = jnp.repeat(init_state[None], args.total_parallel_generations, axis=0, out_sharding=NamedSharding(mesh, P('data')))
init_states = shard_map(partial(jnp.repeat, repeats=args.parallel_generations_per_gpu, axis=0), mesh=mesh, in_specs=P(), out_specs=P('data'))(init_state[None])



# _key = shard_map(jax.random.key, mesh=mesh, in_specs=P(), out_specs=P())(args.seed)
_key = replicate_matrix(jax.random.key(args.seed))
# _key = jax.make_array_from_single_device_arrays(_key.shape, NamedSharding(mesh, P()), [jax.device_put(_key, d) for d in jax.local_devices()])
base_gen_key, base_model_key = jax.random.split(_key)

base_model_tree_keys = generate_base_model_tree_keys(params, base_model_key)

batch_get_keys_for_generation = shard_map(jax.vmap(single_generate_keys_for_generation, in_axes=(None, 0, None)), mesh=mesh, in_specs=(P(), P('data'), P()), out_specs=P('data'))
batch_get_keys_for_model = shard_map(jax.vmap(single_generate_model_tree_keys, in_axes=(None, 0, None)), mesh=mesh, in_specs=(P(), P('data'), P()), out_specs=P('data'))

# sharded_thread_ids = jax.device_put(np.arange(args.total_parallel_generations), NamedSharding(mesh, P('data')))
sharded_thread_ids = jax.make_array_from_single_device_arrays((args.total_parallel_generations,), NamedSharding(mesh, P('data')), [jax.device_put(np.arange(args.parallel_generations_per_gpu) + args.parallel_generations_per_gpu * d.id, d) for d in jax.local_devices()])
common_thread_ids = replicate_matrix(jnp.arange(total_parallel_generations))


model_keys = batch_get_keys_for_model(base_model_tree_keys, sharded_thread_ids, 0)
gen_keys = batch_get_keys_for_generation(base_gen_key, sharded_thread_ids, 0)

# def simple_example(k, x, a):
#     return jax.random.fold_in(k, a*args.total_parallel_generations + x)
#     # return jax.random.key_data(k)[0] + x + 1 + a

# batch_simple_example = shard_map(jax.vmap(simple_example, in_axes=(None, 0, None)), mesh=mesh, in_specs=(P(), P('data'), P()), out_specs=P('data'))
# gen_keys = batch_simple_example(base_gen_key, sharded_thread_ids, replicate_matrix(jnp.array(10)))





# print("gen keys", gen_keys.shape)
# for shard in gen_keys.addressable_shards:
#     print(f"device {shard.device} has local data {shard.data}")


# print("Compiling generate batch")
# start_time = time.time()
# generate_batch = jax.jit(_generate_batch).lower(params, model_keys, gen_keys, jnp.zeros((args.total_parallel_generations, args.generation_length), dtype=jnp.int32)).compile()
...
start_time = time.time()
do_update = jax.jit(_do_update).lower(params, model_keys, jnp.zeros(args.total_parallel_generations), args.lr).compile()
print("Compile time", time.time() - start_time)
print("memory info")
print(do_update.memory_analysis())

pretend_params = do_update(params, model_keys, replicate_matrix(jnp.zeros(args.total_parallel_generations)), args.lr)
print(jax.tree.map(lambda x: x.device, pretend_params))
...
if args.track:
    run = wandb.init(
        ...
        config=args,
        name=args.task+"_"+args.wandb_name+f"_lr={args.lr}_sigma={args.evo_sigma}_bs={args.parallel_generations}"
    )
else:
    print("Run name:", args.task+"_"+args.wandb_name+f"_lr={args.lr}_sigma={args.evo_sigma}_bs={args.parallel_generations}")
...
for epoch in tqdm.trange(args.num_epochs):
    ...
    unique_indices = jnp.arange(args.total_prompts_per_epoch) + epoch * args.total_prompts_per_epoch
    indices = jnp.repeat(unique_indices, args.generations_per_prompt, axis=0)
    batch_prompts = jnp.repeat(task.get_input(unique_indices), args.generations_per_prompt, axis=0)
    ...
    print(batch_prompts.device, output_batch.device, indices.device)
    # print(output_batch)
    ...
    output_scores = jax.block_until_ready(task.get_batch_fitness(indices, output_batch))
    ...
    stats = {
        ...
        "true_train_avg_fitness": true_train_fitness_sum / ((epoch + 1) * args.parallel_generations)
    }
    if args.track:
        run.log(stats)
    else:
