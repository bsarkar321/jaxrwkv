...
from rwkv7_evolution import RWKV7_Evolution, lora_map, FULL, LORA, UNCHANGED
...
_generate_thread, _get_gradient = RWKV7_Evolution(args, RWKV, config)
...
generate_batch = jax.jit(shard_map(
    jax.vmap(_generate_thread, in_axes=(None, 0, 0, None)),
    mesh=mesh,
...
if args.freeze_nonlora:
    solver = optax.masked(solver, jax.tree.map(lambda x: x!=FULL, lora_map))

if args.freeze_lora:
    solver = optax.masked(solver, jax.tree.map(lambda x: x!=LORA, lora_map))
    
optimizer = solver.init(jax.tree.map(lambda p: p.copy(), params))

def _do_update(params, optimizer, raw_scores, epoch_num):
...
do_update = jax.jit(shard_map(
    ...
    in_specs=(P(), P(), P(), P()),
    out_specs=(P(), P(), P())
), donate_argnums=(0, 1)).lower(params, optimizer, jnp.zeros(args.total_parallel_generations), 0).compile()
print("Compile time", time.time() - start_time)
print("memory info")
...
def single_epoch(params, optimizer, true_train_fitness_sum):
    ...
    return params, optimizer, true_train_fitness_sum

for epoch in tqdm.trange(args.num_epochs):
    params, optimizer, true_train_fitness_sum = single_epoch(params, optimizer, true_train_fitness_sum)
