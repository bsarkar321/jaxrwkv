...
class RWKV7_Evolution:

    def __init__(self, args, RWKV, config):
        self.args = args
        self.RWKV = RWKV
        self.config = config
        self.EvoRWKV = get_evo_class(args, RWKV)

    def __hash__(self):
        return hash(self.args)

    def _single_get_iterinfo(self, thread_idx, epoch):
        sigma_antithetic = jnp.where(thread_idx % 2 == 0, self.args.evo_sigma, -self.args.evo_sigma) if self.args.use_antithetic else self.args.evo_sigma
        true_thread_idx = thread_idx // 2 if self.args.use_antithetic else thread_idx
        return epoch, sigma_antithetic, true_thread_idx

    def forward_and_sample(self, model, model_keys, input_token, input_state, generation_key, iterinfo):
        print("compiling forward and sample")
        gen_key, _gen_key = jax.random.split(generation_key)
        generated_outs, generated_state = self.EvoRWKV.evo_forward(model, model_keys, input_token, input_state, iterinfo)
        sampled_tok = jax.random.categorical(_gen_key, generated_outs[-1])
        return sampled_tok, generated_state, gen_key

    def generate_thread(self, params, prompt, thread_idx, epoch_num):
        print("compiling generate_batch")
        
        key = jax.random.key(self.args.seed)
        ...
        iterinfo = self._single_get_iterinfo(thread_idx, epoch_num)
        def inner_scan(carry, input_token):
            tok, state, gen_key = carry
            true_input = jnp.where(input_token == 0, tok, input_token)
            tok, state, gen_key = self.forward_and_sample(params, model_tree_key, true_input, state, gen_key, iterinfo)
            return (tok, state, gen_key), true_input

        init_state = self.RWKV.default_state(params, config=self.config)

        _, out_tokens = jax.lax.scan(inner_scan, (0, init_state, start_gen_key), prompt)
        return out_tokens

    def _simple_full_update(self, param, key, scores, lr, iterinfo):
        if self.args.freeze_nonlora:
            return param
    ...
    def _simple_lora_update(self, param, key, scores, lr, iterinfo):
        if self.args.freeze_lora:
            return param
        # a, b = param.shape
        # epoch, sigma, true_thread_idx = iterinfo
        # true_key = jax.vmap(fold_in_helper, in_axes=(None, 0, 0))(key, epoch, true_thread_idx)
        # noises = jax.vmap(partial(jax.random.normal, shape=(a+b, self.args.lora_dim), dtype=param.dtype))(true_key)
        # Bs = noises[:, :b]
        # As = jnp.ones_like(noises[:, b:])
        epoch, sigma, true_thread_idx = iterinfo
        At, Bt, Ac, Bc = jax.vmap(get_lora_update_params, in_axes=(0, None, None, None))(iterinfo, param, key, self.args.lora_dim)
        broadcasted_scores = jnp.reshape(scores, scores.shape + (1,) * len(param.shape))
    ...
    def do_update(self, params, raw_scores, epoch_num):
        print("compiling do_update")

        print(raw_scores.shape)

        if self.args.group_relative_fitness:
            group_scores = raw_scores.reshape((-1, self.args.generations_per_prompt))
        ...
        batch_iterinfo = jax.vmap(self._single_get_iterinfo, in_axes=(0, None))(jnp.arange(raw_scores.size), epoch_num)
        
        def inner_update(param, model_key, lora_map_ans):
            if lora_map_ans == UNCHANGED:
                return param, 0.0

            update_fn = [self._simple_full_update, self._simple_lora_update][lora_map_ans - 1]

            if len(model_key.shape) == 0:
                new_param = update_fn(param, model_key, true_scores, self.args.lr, batch_iterinfo)
            else:
                new_param = jax.lax.scan(lambda _, x: (0, update_fn(x[0], x[1], true_scores, self.args.lr, batch_iterinfo)), 0, xs=(param, model_key))[1]
            return param, jnp.mean(jnp.abs(param-new_param))
