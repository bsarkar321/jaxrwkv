...
def single_example_loss(params, tokens, advantage):
    T = tokens.shape[0]
    ...
    pi, state = RWKV.forward(params, input_tokens, RWKV.default_state(params, config), config=config)

    pi_logprob = jax.nn.log_softmax(pi[:T])[jnp.arange(T), tokens]
    
    return jnp.mean(pi_logprob * advantage)

def batch_loss(params, batch_tokens, advantages):
    return jnp.mean(jax.vmap(single_example_loss, in_axes=(None, 0, 0))(params, batch_tokens, advantages))
...
def _do_update(params, generations, raw_scores, lr):
    ...
    def update_loop(accum_grads, x):
        gen_batch, score_batch = x
        loss, grad = fast_batch_grad(params, gen_batch, score_batch)
        return jax.tree.map(lambda x, y: x + y, accum_grads, grad), loss

    grad, losses = jax.lax.scan(update_loop, jax.tree.map(lambda x: jnp.zeros_like(x), params), (generations.reshape(args.num_minibatches, args.parallel_generations // args.num_minibatches, -1), true_scores.reshape(args.num_minibatches, args.parallel_generations // args.num_minibatches)))
    params = jax.tree.map(lambda x, y, do_change: (x + lr * y * (do_change != UNCHANGED)).astype(x.dtype), params, grad, lora_map)
    return params
